...
### 3-2. (선택) Fork에 라이브 주소 기록하기

원작자가 **"새 버전이 본인 배포에 잘 반영됐는지"를 원격에서 확인**하고, 뒤처지면 먼저 도와줄 수 있게 하는 단계입니다. 한 번만 하면 됩니다.
...
> [!NOTE]
> 노출되는 정보는 앱이 이미 공개하는 **버전 번호(`/api/version`)** 뿐입니다. 노트 내용·비밀번호 등은 일절 관계없습니다.
> 건너뛰어도 앱 동작에는 전혀 지장이 없습니다 — 원작자 모니터링 편의를 위한 단계입니다.
